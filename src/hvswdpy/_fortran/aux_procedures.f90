      ...
      gam_val=-i*w/alfa*sqrt(cmplx(1.0_long_float - (alfa*alfa)/(c*c), KIND=long_cmplx))
      end function
      ...
      IF(slow.NE.slowP)THEN
        gam_val_from_slow=-i*w*slowP*sqrt(cmplx(1.0_long_float - (slow*slow)/(slowP*slowP), KIND=long_cmplx))
        ! single precision alternative left as comment intentionally
      ELSE
        gam_val_from_slow=-i*w*slowP*sqrt(cmplx(-tiny(0.0_long_float), KIND=long_cmplx))
      ENDIF
      end function
      ...
      nu_val=-i*w/bta*sqrt(cmplx(1.0_long_float - (bta*bta)/(c*c), KIND=long_cmplx))
      end function   
      ...
      IF(slow.NE.slowS)THEN
        nu_val_from_slow=-i*w*slowS*sqrt(cmplx(1.0_long_float - (slow*slow)/(slowS*slowS), KIND=long_cmplx))
        ! single precision alternative left as comment intentionally
      ELSE
        nu_val_from_slow=-i*w*slowS*sqrt(cmplx(-tiny(0.0_long_float), KIND=long_cmplx))
      ENDIF
      end function
